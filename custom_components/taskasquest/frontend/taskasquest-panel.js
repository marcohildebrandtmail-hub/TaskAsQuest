class TaskAsQuestPanel extends HTMLElement {
  ...
  _entityOptions() { return Object.entries(this._hass?.states || {}).map(([id,state]) => `<option value="${this._esc(id)}">${this._esc(state.attributes.friendly_name || id)} — ${this._esc(state.state)}</option>`).sort().join(""); }
  ...
  _dueDateLabel(offset) {
    const map = {"-1":"","0":"Fällig: Heute","1":"Fällig: Morgen","2":"Fällig: In 2 Tagen","3":"Fällig: In 3 Tagen","7":"Fällig: In 7 Tagen","100":"Fällig: Auto"};
    return map[String(offset ?? "-1")] || "";
  }

  _rule(r) { const encoded=encodeURIComponent(JSON.stringify(r)); const on=r.enabled!==false; const due=this._dueDateLabel(r.due_date_offset); return `<div class="rule ${on?'':'off'}"><div><div class="title">${this._esc(r.task_title)}</div><div class="meta">${this._esc(r.entity_id)} ist ${this._condition(r)} ${this._esc(r.value)} · ${this._esc(r.difficulty)} · ${r.cooldown} Min. Cooldown${due?' · '+this._esc(due):''}</div></div><div class="actions"><button class="quiet" data-toggle="${r.id}" data-entry="${r.entry_id}" data-enabled="${on}">${on?'Aktiv':'Pausiert'}</button><button class="quiet" data-edit="${encoded}">Bearbeiten</button><button class="quiet danger" data-delete="${r.id}" data-entry="${r.entry_id}">Löschen</button></div></div>`; }
  ...
  _openEditor(rule={}) {
    ...
    const choices=(label,name,value,items)=>`<label>${label}<select name="${name}">${items.map(([v,l])=>`<option value="${v}" ${String(value)===String(v)?'selected':''}>${l}</option>`).join("")}</select></label>`;
    ...
    const dueDateChoices=[["-1","Keine Fälligkeit"],["0","Heute (23:59)"],["1","Morgen (23:59)"],["2","In 2 Tagen"],["3","In 3 Tagen"],["7","In 7 Tagen"],["100","Auto (Heute / ab 18 Uhr Morgen)"]];
    ...
    const entityValues=String(rule.entity_id||"").split(",").map(value=>value.trim());
    const entityField=(index)=>`<label class="wide">Auslösende Entity ${index + 1}<input name="entity_${index}" list="taskasquest-entities" value="${this._esc(entityValues[index]||"")}" placeholder="Name oder Entity-ID suchen"><datalist id="taskasquest-entities">${this._entityOptions()}</datalist></label>`;
    const overlay=document.createElement("div"); overlay.className="overlay";
    overlay.innerHTML=`<form class="dialog"><h2>${rule.id?'Regel bearbeiten':'Neue Automation'}</h2><div class="form">${field("Quest-Titel","task_title",rule.task_title||"",true)}${entityField(0)}${entityField(1)}${entityField(2)}${choices("Bedingung","condition",rule.condition||"equals",[["equals","Gleich"],["not_equals","Ungleich"],["below","Kleiner als"],["above","Größer als"]])}${field("Auslösewert","value",rule.value||"")}${choices("Schwierigkeit","difficulty",rule.difficulty||"medium",[["easy","Leicht"],["medium","Mittel"],["hard","Schwer"],["epic","Episch"]])}${field("Cooldown (Minuten)","cooldown",rule.cooldown??1440)}${choices("Auslöseverhalten","trigger_mode",rule.trigger_mode||"edge",[["edge","Nur bei Änderung"],["level","Wiederholen"]])}${choices("Fälligkeit","due_date_offset",String(rule.due_date_offset??"-1"),dueDateChoices)}${assigneesField}<label class="wide"><input type="checkbox" name="enabled" ${rule.enabled!==false?'checked':''}> Regel aktiviert</label></div><div class="dialog-actions"><button type="button" class="quiet">Abbrechen</button><button type="submit">Speichern</button></div></form>`;
    overlay.querySelector("button[type=button]").onclick=()=>overlay.remove(); overlay.onclick=e=>{if(e.target===overlay) overlay.remove();};
    overlay.querySelector("form").onsubmit=e=>{e.preventDefault(); const values=new FormData(e.currentTarget); const assigneesData=values.getAll("assignees"); const parsedAssignees=assigneesData.length===1&&typeof assigneesData[0]==="string"&&assigneesData[0].includes(",")?assigneesData[0].split(",").map(s=>s.trim()).filter(Boolean):assigneesData.filter(Boolean); const selectedEntities=[0,1,2].map(index=>String(values.get(`entity_${index}`)||"").trim()).filter(Boolean); if(!selectedEntities.length){overlay.querySelector("[name=entity_0]").focus(); return;} const updated={...rule,entity_id:selectedEntities.join(", "),task_title:values.get("task_title"),value:values.get("value"),condition:values.get("condition"),difficulty:values.get("difficulty"),cooldown:Number(values.get("cooldown")),trigger_mode:values.get("trigger_mode"),due_date_offset:values.get("due_date_offset")||"-1",assignees:parsedAssignees,enabled:values.get("enabled") === "on",notify_app:rule.notify_app!==false}; overlay.remove(); this._request({action:rule.id?"update":"create",entry_id,rule:updated});};
