class TaskAsQuestPanel extends HTMLElement {
  ...
  _openEditor(rule={}) {
    ...
    const field=(label,name,value,wide=false)=>`<label class="${wide?'wide':''}">${label}<input name="${name}" value="${this._esc(value)}" required></label>`;
    ...
    overlay.innerHTML=`<form class="dialog"><h2>${rule.id?'Regel bearbeiten':'Neue Automation'}</h2><p class="hint">Wähle die auslösende Entität aus deiner Home-Assistant-Übersicht aus.</p><div class="form">${field("Quest-Titel","task_title",rule.task_title||"",true)}<label class="wide">Auslösende Home-Assistant-Entität<input name="entity_id" list="taskasquest-entities" value="${this._esc(rule.entity_id||"")}" placeholder="Nach Name oder Entity-ID suchen" required><datalist id="taskasquest-entities">${this._entityOptions()}</datalist></label>${choices("Bedingung",rule.condition||"equals",[["equals","Gleich"],["not_equals","Ungleich"],["below","Kleiner als"],["above","Größer als"]])}${field("Auslösewert","value",rule.value||"")}${choices("Schwierigkeit",rule.difficulty||"medium",[["easy","Leicht"],["medium","Mittel"],["hard","Schwer"],["epic","Episch"]])}${field("Cooldown (Minuten)","cooldown",rule.cooldown??1440)}${choices("Auslöseverhalten",rule.trigger_mode||"edge",[["edge","Nur bei Änderung"],["level","Wiederholen, solange wahr"]])}<label><input type="checkbox" name="enabled" ${rule.enabled!==false?'checked':''}> Regel aktiviert</label></div><div class="dialog-actions"><button type="button" class="quiet">Abbrechen</button><button type="submit">Speichern</button></div></form>`;
    overlay.querySelector("button[type=button]").onclick=()=>overlay.remove(); overlay.onclick=e=>{if(e.target===overlay) overlay.remove();};
    overlay.querySelector("form").onsubmit=e=>{e.preventDefault(); const values=new FormData(e.currentTarget); const updated={...rule,entity_id:values.get("entity_id"),task_title:values.get("task_title"),value:values.get("value"),condition:values.get("condition"),difficulty:values.get("difficulty"),cooldown:Number(values.get("cooldown")),trigger_mode:values.get("trigger_mode"),enabled:values.get("enabled") === "on",notify_app:rule.notify_app!==false}; overlay.remove(); this._request({action:rule.id?"update":"create",entry_id,rule:updated});};
